...
func (c *Client) MakeRestRequest(method string, path string, body *container.Container, authenticated bool) (*http.Request, error) {
	if c.platform == "nd" && path != "/login" {
		...
		// TODO: Need to update the path assignment logic to support ND
...
func (c *Client) Do(req *http.Request) (*container.Container, *http.Response, error) {
	...
	err = resp.Body.Close()
	if err != nil {
		log.Printf("\n\n -------------- Error occured while closing body %v", err)
		return nil, nil, err
	}
	// defer resp.Body.Close()
	log.Printf("\n\n -------------- resp.StatusCode: %v", resp.StatusCode)

	log.Printf("[DEBUG] HTTP response unique string method: %s url: %s body_str: %v", req.Method, req.URL.String(), bodyStr)
	if req.Method != "DELETE" && resp.StatusCode != 204 {
		log.Printf("\n\n -------------- Before parseJons %v", bodyBytes)

		obj, err := container.ParseJSON(bodyBytes)
		log.Printf("\n\n -------------- After parseJons %v", obj)
