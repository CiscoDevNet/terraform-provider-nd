...
func TestResourceFabricVxlanEbgp(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_fabric_vxlan_ebgp",
			args: args{
				rscType:  "resource",
				rscName:  "fabric_vxlan_ebgp",
				dataFile: "/examples/ndfc_payloads/data_fabric_vxlan_ebgp.json",
			},
		},
	}
