...
func TestResourceFabricExternal(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_fabric_external",
			args: args{
				rscType:  "resource",
				rscName:  "fabric_external",
				dataFile: "/examples/ndfc_payloads/data_fabric_external.json",
			},
		},
	}
