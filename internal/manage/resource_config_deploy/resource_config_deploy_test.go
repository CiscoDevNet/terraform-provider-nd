...
func TestResourceConfigDeploy(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_config_deploy",
			args: args{
				rscType:  "resource",
				rscName:  "config_deploy",
				dataFile: "/examples/ndfc_payloads/data_config_deploy.json",
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name+"model_read", func(t *testing.T) {
			...
			modelData := NDFCConfigDeployModel{}
			v := ConfigDeployModel{}
