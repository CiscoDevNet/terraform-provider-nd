...
func TestResourceFabricVxlanIbgp(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_fabric_vxlan_ibgp",
			args: args{
				rscType:  "resource",
				rscName:  "fabric_vxlan_ibgp",
				dataFile: "/examples/ndfc_payloads/data_fabric_vxlan_ibgp.json",
			},
		},
	}
