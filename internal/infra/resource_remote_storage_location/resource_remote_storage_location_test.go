...
func TestResourceRemoteStorageLocation(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_remote_storage_location",
			args: args{
				rscType:  "resource",
				rscName:  "remote_storage_location",
				dataFile: "/examples/ndfc_payloads/data_remote_storage_location.json",
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name+"model_read", func(t *testing.T) {
			...
			modelData := NDFCRemoteStorageLocationModel{}
			v := RemoteStorageLocationModel{}
